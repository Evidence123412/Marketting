<template>
  <div class="space-y-6 h-full flex flex-col">
    ...
    >
      ...
      <p class="text-gray-500 mt-1">Personaliza tu experiencia, gestiona tu marca y configura el sistema.</p>
    </div>
    ...
    <div class="flex-1 overflow-y-auto scrollbar-thin pr-2 pb-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Briefcase :size="20" class="text-kapital-dark" /> Identidad de Marca
            </h2>
            <p class="text-sm text-gray-500 mt-1">Define cómo se presenta tu marca en el contenido generado.</p>
          </div>
            ...
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <button 
                  ...
                  :class="['p-3 border rounded-xl text-sm font-medium transition-all text-center', branding.tone === tone.id ? 'bg-kapital-dark text-white border-kapital-dark' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300']"
                >
                  {{ tone.label }}
                </button>
              </div>
            ...
            <div class="md:col-span-2">
              ...
              <div class="flex gap-4">
                <div v-for="(color, index) in branding.colors" :key="index" class="flex flex-col gap-2">
                  ...
                  <span class="text-xs font-mono text-gray-500 uppercase">{{ branding.colors[index] }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end pt-6 mt-6 border-t border-gray-100">
            <button @click="saveGeneral" class="btn-primary shadow-lg shadow-gray-900/20">
      ...
      <div v-if="activeTab === 'integrations'" class="space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div class="flex justify-between items-center mb-6">
            <div>
              <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                <LinkIcon :size="20" class="text-kapital-dark" /> Redes Sociales
              </h2>
              <p class="text-sm text-gray-500 mt-1">Conecta tus cuentas para publicar automáticamente.</p>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            <div 
              ...
              :class="[
                ...
                network.connected
                  ? 'border-kapital-dark bg-gray-50 ring-1 ring-kapital-dark/10'
                  : 'border-gray-200 bg-white hover:border-kapital-dark/50 hover:shadow-md'
              ]"
            >
              ...
              <div class="flex-1 relative z-10">
                <p class="font-bold text-gray-900">{{ network.name }}</p>
              ...
              <div :class="['w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors relative z-10', network.connected ? 'bg-kapital-dark border-kapital-dark' : 'border-gray-300 bg-gray-50']">
          ...
          <div class="flex justify-end pt-6 border-t border-gray-100">
            <button @click="saveNetworks" class="btn-primary shadow-lg shadow-gray-900/20">
      ...
      <div v-if="activeTab === 'variables'" class="space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Database :size="20" class="text-kapital-dark" /> Variables del Sistema
            </h2>
            <p class="text-sm text-gray-500 mt-1">Configura las opciones disponibles para la generación de Buyer Persona.</p>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <button 
                ...
                :class="[
                  ...
                  activeVariableList === list.id 
                    ? 'bg-kapital-dark text-white shadow-md shadow-gray-900/20' 
            ...
            <!-- Content Editor -->
            <div class="lg:col-span-2 bg-gray-50 rounded-xl p-4 border border-gray-200">
              ...
              <div class="space-y-2 max-h-[400px] overflow-y-auto scrollbar-thin pr-2">
                <div 
                  ...
                  :key="index"
                  class="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-200 group hover:border-kapital-dark/30 transition-colors"
                >
                  <span class="text-sm font-medium text-gray-700">{{ item }}</span>
                  <button 
                    @click="removeVariable(activeVariableList, item)"
                    class="text-gray-400 hover:text-rose-500 p-1 rounded-md hover:bg-rose-50 transition-colors"
                  >
                    <Trash2 :size="16" />
                  </button>
                </div>
                <div v-if="variableLists.find(l => l.id === activeVariableList).data.value.length === 0" class="text-center py-8 text-gray-400">
      ...
      <div v-if="activeTab === 'ai'" class="space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Wand2 :size="20" class="text-kapital-dark" /> IA y Automatización
            </h2>
            <p class="text-sm text-gray-500 mt-1">Configura el comportamiento del generador de contenido y la programación.</p>
          </div>

          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- AI Settings -->
            <div class="space-y-6">
              <h3 class="font-bold text-gray-900 text-sm uppercase tracking-wide border-b border-gray-100 pb-2">Preferencias de IA</h3>
              
              <div>
                <label class="form-label flex justify-between">
                  <span>Nivel de Creatividad</span>
                  <span class="text-kapital-dark">{{ aiSettings.creativity }}%</span>
                </label>
                <input v-model="aiSettings.creativity" type="range" min="0" max="100" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-kapital-dark" />
            ...
            <!-- Schedule Settings -->
            <div class="space-y-6">
              <h3 class="font-bold text-gray-900 text-sm uppercase tracking-wide border-b border-gray-100 pb-2">Programación Automática</h3>
              ...
              <div class="bg-gray-50 p-4 rounded-xl border border-gray-200">
                <label class="flex items-center gap-3 cursor-pointer">
                  <input v-model="schedule.autoPublish" type="checkbox" class="w-5 h-5 text-kapital-dark rounded border-gray-300 focus:ring-kapital-dark" />
                  <div>
                    <span class="block font-bold text-gray-900 text-sm">Publicación Automática</span>
                    <span class="block text-xs text-gray-500">Publicar sin revisión manual si la confianza de la IA es alta.</span>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div class="flex justify-end pt-6 mt-6 border-t border-gray-100">
            <button @click="saveAI" class="btn-primary shadow-lg shadow-gray-900/20">
      ...
      <div v-if="activeTab === 'notifications'" class="space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Bell :size="20" class="text-kapital-dark" /> Notificaciones
            </h2>
            <p class="text-sm text-gray-500 mt-1">Elige qué alertas deseas recibir.</p>
          </div>

          <div class="space-y-4">
            <div v-for="notif in notifications" :key="notif.id" class="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-bold text-gray-900 text-sm">{{ notif.label }}</p>
              ...
              <label class="relative inline-flex items-center cursor-pointer">
                ...
                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-gray-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-kapital-dark"></div>
              </label>
            </div>
          </div>

          <div class="flex justify-end pt-6 mt-6 border-t border-gray-100">
            <button @click="saveNotifications" class="btn-primary shadow-lg shadow-gray-900/20">
      ...
      <div v-if="activeTab === 'team'" class="space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            <div>
              <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                <Users :size="20" class="text-kapital-dark" /> Equipo
              </h2>
              <p class="text-sm text-gray-500 mt-1">Gestiona los miembros de tu equipo y sus permisos.</p>
            </div>
            <button @click="showNewUserModal = true" class="btn-primary shadow-lg shadow-gray-900/20">
            ...
            <table class="w-full text-sm text-left">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50/50 transition-colors group">
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-full bg-gray-900 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                        {{ user.initials }}
                      </div>
                  ...
                  <td class="px-6 py-4">
                    <span :class="['text-xs font-bold px-2.5 py-1 rounded-full inline-flex items-center gap-1.5', user.active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500']">
                      <div :class="['w-1.5 h-1.5 rounded-full', user.active ? 'bg-emerald-500' : 'bg-gray-400']"></div>
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        @click="editUser(user)"
                        class="p-2 text-gray-400 hover:text-kapital-dark hover:bg-gray-100 rounded-lg transition-colors"
                        title="Editar"
                      >
                        <Edit2 :size="16" />
                      </button>
                      <button 
                        ...
                        :class="['p-2 rounded-lg transition-colors', user.active ? 'text-gray-400 hover:text-amber-600 hover:bg-amber-50' : 'text-gray-400 hover:text-emerald-600 hover:bg-emerald-50']"
    ...
    <!-- New User Modal -->
    <div v-if="showNewUserModal" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div 
          v-motion
        ...
        >
          <div class="flex justify-between items-center mb-6">
            ...
            <button @click="showNewUserModal = false" class="text-gray-400 hover:text-gray-600 p-1 hover:bg-gray-100 rounded-lg transition-colors">
              <X :size="24" />
            </button>
          </div>
        ...
        <form @submit.prevent="addUser" class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <Shield :size="16" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          ...
          <div class="flex gap-3 pt-6 border-t border-gray-100">
            ...
            <button 
              type="submit"
              class="btn-primary flex-1 shadow-lg shadow-gray-900/20"
            >
...
const networks = ref([
  ...
  { id: 3, name: 'Twitter', iconComponent: Twitter, bgClass: 'bg-gray-900', connected: false, followers: '-' },
...
function getRoleBadge(role) {
  const classes = {
    ...
    'Visualizador': 'bg-gray-50 text-gray-700 border-gray-200'
  }
...
.form-label {
  @apply block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5;
}
.form-input {
  @apply w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-kapital-dark focus:ring-2 focus:ring-kapital-dark/20 transition-all bg-white text-sm text-gray-700;
}

.btn-primary {
  @apply px-5 py-2.5 bg-kapital-dark text-white font-medium rounded-xl transition-all hover:bg-gray-700 hover:shadow-lg hover:shadow-gray-900/30 active:scale-95 flex items-center gap-2 justify-center text-sm;
}
